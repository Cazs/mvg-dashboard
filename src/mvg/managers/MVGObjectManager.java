 ...
 */
public abstract class MVGObjectManager
{
    private MVGObject selected;
    ...
    public abstract HashMap<String, ? extends MVGObject> getDataset();

    public void setSelected(MVGObject selected)
    {
        this.selected=selected;
    }

    public MVGObject getSelected()
    {
        return this.selected;
    }
    
    public void emailMVGObject(MVGObject MVGObject, String pdf_path, Callback callback) throws IOException
    {
        if(MVGObject==null)
        {
            IO.logAndAlert("Error", "Invalid "+MVGObject.getClass().getName(), IO.TAG_ERROR);
            return;
        }
        ...
        } else IO.log(QuoteManager.class.getName(), "Could not get valid path for created "+MVGObject.getClass().getName()+" pdf.", IO.TAG_ERROR);
        final String finalBase64_obj = base64_obj;

        Stage stage = new Stage();
        stage.setTitle(Globals.APP_NAME.getValue() + " - eMail "+MVGObject.getClass().getSimpleName()+" ["+MVGObject.get_id()+"]");
        ...
        txt_subject.setText(MVGObject.getClass().getSimpleName()+" ["+MVGObject.get_id()+"]");
        ...
        submit = CustomTableViewControls.getSpacedButton("Send", event ->
        {
            ...
            try
            {
                ...
                headers.add(new AbstractMap.SimpleEntry<>("_id", MVGObject.get_id()));
                ...
                FileMetadata fileMetadata = new FileMetadata(MVGObject.getClass().getSimpleName().toLowerCase()+"_"+MVGObject.get_id()+".pdf","application/pdf");
                fileMetadata.setCreator(SessionManager.getInstance().getActive().getUsername());
                fileMetadata.setFile(finalBase64_obj);
                HttpURLConnection connection = RemoteComms.postJSON(MVGObject.apiEndpoint()+"/mailto", fileMetadata.asJSONString(), headers);
                if(connection!=null)
                {
                    if(connection.getResponseCode()==HttpURLConnection.HTTP_OK)
                    {
                        //TODO: CC self
                        IO.logAndAlert("Success", "Successfully emailed "+MVGObject.getClass().getSimpleName()+"!", IO.TAG_INFO);
                        if(callback!=null)
                            callback.call(null);
                    } else {
